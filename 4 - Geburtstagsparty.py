...
f = 0
jc = 0
for i in range(0, len(geburtstage)):
    list = geburtstage[i]
    tag = str(list[0])
    if tag == "17":
        ö += 1
    ...
    if monat == "3":
        ä +=1
    monat_iso.append(monat)
    jahr = str(list[2])
    if jahr == "1997":
        ü += 1
    jahr_iso.append(jahr)
    datum = tag + "." + monat + "." + jahr
    reinform.append(datum)
    i += 1
if len(geburtstage) == 345:
    print("Gut gezählt.")
else:
    print("Schade, du hast", len(geburtstage), "aufgeschrieben.")

for i in monat_iso:
    stat[int(i) - 1] += 1

maxes = [index for index, value in enumerate(stat) if value == max(stat)]
maxes = [k + 1 for k in maxes]
...
print("Die 142. Person hat am", reinform[141], "Geburtstag")
print(ä, "Gäste haben im März Geburtstag.")
print(ö, "Gäste haben an einem 17. Geburtstag")
print(ü, "Gäste wurden 1997 geboren.")
print("Die meisten Gäste haben in den Monaten", maxes[0:len(maxes)], "Geburtstag")
